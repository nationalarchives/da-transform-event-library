...
if __name__ == "__main__":
    ...
    about = json.loads(pkgutil.get_data(package=__name__, resource=ABOUT).decode())
...
def get_event_schema(event_name: str = ROOT_EVENT) -> str:
    ...
    schema = json.loads(
        pkgutil.get_data(package=__name__, resource=schema_resource).decode()
    )
...
def get_schema_store() -> dict:
    ...
    for event in event_list:
        ...
        if KEY_SCHEMA_ID not in schema:
            raise ValueError(
                f'Key "{KEY_SCHEMA_ID}" not found in schema for ' f'event "{event}"'
            )
...
def validate_event(event: dict, schema_name: str = None):
    """
    ...
    resolver = jsonschema.RefResolver.from_schema(
        schema=schema, store=get_schema_store()
    )

    jsonschema.Draft202012Validator(schema=schema, resolver=resolver).validate(event)
